...
The node scenario keeps the GIS claim precise: unavailable reference points
are screened against the next-nearest governed node with Haversine distance.
It does not present the result as a shipment lane, route plan, capacity promise,
or execution approval.

![GIS node scenario showing Ontario DC 1 unavailable, six changed supplier screens, the transparent distance deltas, and the governed world map.](docs/assets/gis-decision-node-scenario.png)
...
eligible external alternate, and 7 of 23 whose eligible alternate lead time
...
# 7. launch the interactive GIS Network Risk Decision Assurance Studio
pip install -r requirements.txt
streamlit run streamlit_app.py
...
app/                 tested disruption, next-nearest screening, assurance and evidence-pack logic
streamlit_app.py     Network Risk Decision Assurance Studio
